...
def visualize_full_graph_name(g, rb_vec, eval_env, success_rate, eval_time, cfg, type):
    plt.figure(figsize=(6, 6))
    plot_walls(eval_env.walls)
    plt.scatter(rb_vec[g.nodes, 0], rb_vec[g.nodes, 1])
    ...
    plt.title(f'|V|={g.number_of_nodes()}, |E|={len(edges_to_plot)}')
    filename = str(cfg.env.env_name) + "_succrate_" + str(round(success_rate, 3)).replace(".", "-") + "_eval_" + str(round(eval_time, 3)).replace(".", "-") + "_E_" + str(len(edges_to_plot)) +  "_V_" + str(g.number_of_nodes()) + "_" + str(type) + ".png"
    plt.savefig(os.path.join(cfg.ckpt_dir, filename))
    # plt.show()
