...
def cleanup_and_eval_search_policy(search_policy, eval_env, num_evals=10, difficulty=0.5, cleanup=int(1e5)):
    set_env_difficulty(eval_env, difficulty)
    search_policy.reset_stats()
    initial_success_rate, initial_eval_time = eval_search_policy(search_policy, eval_env, num_evals=num_evals)

    # Initial sparse graph
    print(f'Initial {search_policy} has success rate {initial_success_rate:.2f}, evaluated in {initial_eval_time:.2f} seconds')
    ...
    set_env_difficulty(eval_env, difficulty)
    search_policy.reset_stats()
    filtered_success_rate, filtered_eval_time = eval_search_policy(search_policy, eval_env, num_evals=num_evals)
    print(f'Filtered {search_policy} has success rate {filtered_success_rate:.2f}, evaluated in {filtered_eval_time:.2f} seconds')
    filtered_g, filtered_rb = search_policy.g.copy(), search_policy.rb_vec.copy()

    # Cleanup steps
    num_cleanup_steps = cleanup
    ...
    set_env_difficulty(eval_env, difficulty)
    search_policy.reset_stats()
    cleaned_success_rate, cleaned_eval_time = eval_search_policy(search_policy, eval_env, num_evals=num_evals)
    print(f'Cleaned {search_policy} has success rate {cleaned_success_rate:.2f}, evaluated in {cleaned_eval_time:.2f} seconds')
    cleaned_g, cleaned_rb = search_policy.g.copy(), search_policy.rb_vec.copy()

    return (initial_g, initial_rb, initial_success_rate, initial_eval_time), \
        (filtered_g, filtered_rb, filtered_success_rate, filtered_eval_time), \
        (cleaned_g, cleaned_rb, cleaned_success_rate, cleaned_eval_time, cleanup_time)


def cleanup_and_eval_search_policy_sorb(search_policy, eval_env, num_evals=10, difficulty=0.5):
    set_env_difficulty(eval_env, difficulty)
    search_policy.reset_stats()
    initial_success_rate, initial_eval_time = eval_search_policy(search_policy, eval_env, num_evals=num_evals)
    ...
    return (initial_g, initial_rb, initial_success_rate, initial_eval_time), (filtered_g, filtered_rb, filtered_success_rate, filtered_eval_time)
